...
def positional(max_positional_args):
  ...
  if isinstance(max_positional_args, six.integer_types):
    ...
    spec = inspect.getfullargspec(max_positional_args)
    args = spec.args
...
class AcceptItem(object):
  ...
  def __eq__(self, other):
    """Equality operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key == other.sort_key

  def __ne__(self, other):
    """Non-equality operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key != other.sort_key

  def __lt__(self, other):
    """Less than operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key < other.sort_key

  def __le__(self, other):
    """Less than or equal operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key <= other.sort_key

  def __gt__(self, other):
    """Greater than operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key > other.sort_key

  def __ge__(self, other):
    """Greater than or equal operator based on sort keys."""
    if not isinstance(other, AcceptItem):
      return NotImplemented
    return self.sort_key >= other.sort_key
  ...
  def __str__(self):
    ...
    if values:
      value_strings = ['%s=%s' % (i, v) for i, v in six.iteritems(values)]
...
def parse_accept_header(accept_header):
  ...
  """
  accept_items = []
  for index, header in enumerate(six.ensure_str(accept_header).split(',')):
...
def decode_datetime(encoded_datetime):
  """Decode a datetime field value from a string or a dict to a python datetime.

  Args:
    encoded_datetime: A string in RFC 3339 format or a dict matching
                      the definition of DateTimeField: required key milliseconds
                      and optional key time_zone_offset, both values are int.

  Returns:
    A datetime object with the date and time specified in encoded_datetime.

  Raises:
    TypeError: If the value is neither string nor dict.
  """
  if isinstance(encoded_datetime, six.string_types):
    return _decode_datetime_string(encoded_datetime)
  elif isinstance(encoded_datetime, dict):
    return _decode_datetime_dict(encoded_datetime)
  else:
    raise TypeError(
        'Cannot decode {type}, only can decode string or a dict.'.format(
            type=type(encoded_datetime)))


def _decode_datetime_dict(encoded_datetime):
  """Decode a datetime field value from a dict to a python datetime.

  Args:
    encoded_datetime: A dict matching the definition of DateTimeField: required
                      key milliseconds and optional key time_zone_offset,
                      both values are int.
  ...
  Raises:
    ValueError: If the dict doesn't have milliseconds.
  """
  if 'milliseconds' not in encoded_datetime:
    raise ValueError('Milliseconds is a required key for the datetime.')
  seconds = int(encoded_datetime['milliseconds']) / 1000.0
  time_zone_offset = encoded_datetime.get('time_zone_offset')
  naive_datetime = datetime.datetime.utcfromtimestamp(seconds)
  if time_zone_offset is None:
    result = naive_datetime
  else:
    zero_offset = TimeZoneOffset(0)
    time_zone = TimeZoneOffset(int(time_zone_offset))
    timezone_aware_datetime = naive_datetime.replace(tzinfo=zero_offset)
    result = timezone_aware_datetime.astimezone(time_zone)
  return result


def _decode_datetime_string(encoded_datetime):
  ...
  time_zone_match = _TIME_ZONE_RE.search(six.ensure_str(encoded_datetime))
  if time_zone_match:
