...
class RegistryService(remote.Service):
  ...
  def services(self, request):
    ...
    response.services = []
    for name, service_class in six.iteritems(self.__registry):
      mapping = ServiceMapping()
      mapping.name = six.ensure_text(name, 'utf-8')
      mapping.definition = six.ensure_text(service_class.definition_name(),
                                           'utf-8')
