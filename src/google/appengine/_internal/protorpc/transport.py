...
class HttpTransport(Transport):
  ...
  def __get_rpc_status(self, response, content):
    ...
    if not content.strip():
      content = six.moves.http_client.responses.get(response.status,
                                                    'Unknown Error')
    return remote.RpcStatus(
        state=remote.RpcState.SERVER_ERROR,
        error_message='HTTP Error %s: %s' %
        (response.status, six.ensure_str(content) or 'Unknown Error'))
  ...
  def _start_rpc(self, remote_info, request):
    ...
    url = six.moves.urllib.parse.urlparse(method_url)
    if url.scheme == 'https':
  ...
  def _start_rpc(self, remote_info, request):
    ...
    def wait_impl():
      ...
      try:
        ...
        six.reraise(remote.ServerError, remote.ServerError(message), traceback)
      rpc.set_response(response)
    rpc._wait_impl = wait_impl
    return rpc
