...
_USE_GRPC = os.environ.get(USE_CUSTOM_IMAGES_GRPC_SERVICE, "").lower() in ("true", "1")
...
JPEG = images_service_pb2.OutputSettings.JPEG
PNG = images_service_pb2.OutputSettings.PNG
WEBP = images_service_pb2.OutputSettings.WEBP
BMP = -1
GIF = -2
ICO = -3
TIFF = -4
...
UNCHANGED_ORIENTATION = images_service_pb2.InputSettings.UNCHANGED_ORIENTATION
CORRECT_ORIENTATION = images_service_pb2.InputSettings.CORRECT_ORIENTATION

ORIENTATION_CORRECTION_TYPE = frozenset(
    [UNCHANGED_ORIENTATION, CORRECT_ORIENTATION])

TOP_LEFT = images_service_pb2.CompositeImageOptions.TOP_LEFT
TOP_CENTER = images_service_pb2.CompositeImageOptions.TOP
TOP_RIGHT = images_service_pb2.CompositeImageOptions.TOP_RIGHT
CENTER_LEFT = images_service_pb2.CompositeImageOptions.LEFT
CENTER_CENTER = images_service_pb2.CompositeImageOptions.CENTER
CENTER_RIGHT = images_service_pb2.CompositeImageOptions.RIGHT
BOTTOM_LEFT = images_service_pb2.CompositeImageOptions.BOTTOM_LEFT
BOTTOM_CENTER = images_service_pb2.CompositeImageOptions.BOTTOM
BOTTOM_RIGHT = images_service_pb2.CompositeImageOptions.BOTTOM_RIGHT
...
def _ToImagesError(error, blob_key=None):
  ...
  Args:
    ...
    blob_key: The `blob_key` that is used in the function that caused the error.
      Set this argument to `None` if the blob key is unknown.
  ...
  error_map = {
      ...
      images_service_pb2.ImagesServiceError.BAD_IMAGE_DATA:
          BadImageError,
      images_service_pb2.ImagesServiceError.IMAGE_TOO_LARGE:
          LargeImageError,
      images_service_pb2.ImagesServiceError.INVALID_BLOB_KEY:
          InvalidBlobKeyError,
      images_service_pb2.ImagesServiceError.ACCESS_DENIED:
          AccessDeniedError,
      images_service_pb2.ImagesServiceError.OBJECT_NOT_FOUND:
          ObjectNotFoundError,
  ...
  if error_code == images_service_pb2.ImagesServiceError.INVALID_BLOB_KEY:
...
class Image(object):
  ...
  def __init__(self, image_data=None, blob_key=None, filename=None):
    ...
    Args:
      ...
      filename: String of the file name of a Google Storage file that contains
        the image data. Must be in the format `/gs/bucket_name/object_name`.
  ...
  def _update_png_dimensions(self):
    ...
    size = len(self._image_data)
    if size >= 24 and self._image_data[12:16] == b"IHDR":
  ...
  def _update_jpeg_dimensions(self):
    ...
    offset = 2
    while offset < size:
      while offset < size and self._byte_to_int(
          self._image_data[offset]) != 0xFF:
        offset += 1
      while offset < size and self._byte_to_int(
          self._image_data[offset]) == 0xFF:
        offset += 1
      if (offset < size and
          self._byte_to_int(self._image_data[offset]) & 0xF0 == 0xC0 and
          self._byte_to_int(self._image_data[offset]) != 0xC4):
        offset += 4
        if offset + 4 <= size:
          self._height, self._width = struct.unpack(
              ">HH", self._image_data[offset:offset + 4])
          break
        else:
          ...
          if tag == 0x100 or tag == 0x101:
            ...
            if tag == 0x100:
              self._width = struct.unpack(
                  format, self._image_data[ifd_offset + 8:end_offset])[0]
  ...
  def _update_webp_dimensions(self):
    ...
    bits = self._byte_to_int(self._image_data[20]) | (
        self._byte_to_int(self._image_data[21]) << 8) | (
            self._byte_to_int(self._image_data[22]) << 16)
  ...
  def resize(self,
    ...
    Args:
      ...
      crop_to_fit: If `True`, and both `width` and `height` are specified, the
        image is cropped after it is resized to fit the specified dimensions.
      ...
      allow_stretch: If `True`, and both `width` and `height` are specified, the
        image is stretched to fit the resize dimensions without maintaining the
        aspect ratio.
    ...
    transform = images_service_pb2.Transform()
    transform.width = width
    transform.height = height
    transform.crop_to_fit = crop_to_fit
    transform.crop_offset_x = crop_offset_x
    transform.crop_offset_y = crop_offset_y
    transform.allow_stretch = allow_stretch
  ...
  def crop(self, left_x, top_y, right_x, bottom_y):
    ...
    transform = images_service_pb2.Transform()
    transform.crop_left_x = left_x
    transform.crop_top_y = top_y
    transform.crop_right_x = right_x
    transform.crop_bottom_y = bottom_y
  ...
  def execute_transforms(self,
    ...
    Args:
      ...
      quality: A value between 1 and 100 to specify the quality of the encoding.
        This value is only used for JPEG and WEBP quality control.
      parse_source_metadata: When `True`, the metadata (EXIF) of the source
        image is parsed before any transformations. The results can be retrieved
        via `Image.get_original_metadata`.
      transparent_substitution_rgb: When transparent pixels are not supported in
        the destination image format, transparent pixels will be substituted for
        the specified color, which must be 32-bit RGB format.
    ...
    """
    rpc = self.execute_transforms_async(
  ...
  def execute_transforms_async(self,
                               output_encoding=PNG,
                               quality=None,
                               parse_source_metadata=False,
                               transparent_substitution_rgb=None,
    ...
    Args:
      output_encoding: A value from `OUTPUT_ENCODING_TYPES`.
      quality: A value between 1 and 100 to specify the quality of the encoding.
        This value is only used for JPEG & WEBP quality control.
      parse_source_metadata: When `True`, the metadata (EXIF) of the source
        image is parsed before any transformations. The results can be retrieved
        via `Image.get_original_metadata`.
      transparent_substitution_rgb: When transparent pixels are not supported in
        the destination image format, transparent pixels will be substituted for
        the specified color, which must be in 32-bit RGB format.
      rpc: A UserRPC object.
    ...
    input_settings.correct_exif_orientation = self._correct_orientation

    if parse_source_metadata:
      input_settings.parse_metadata = True
    ...
    def execute_transforms_hook(rpc):
      ...
      self._blob_key = None
      self._transforms = []
      if response.image.HasField("width"):
        self._width = rpc.response.image.width
      else:
        self._width = None
      if response.image.HasField("height"):
    ...
    return _make_async_call(rpc, "Transform", request, response,
                            execute_transforms_hook, None)
  ...
  def histogram_async(self, rpc=None):
    ...
    def get_histogram_hook(rpc):
      ...
      histogram_ = rpc.response.histogram
      return [histogram_.red, histogram_.green, histogram_.blue]

    return _make_async_call(rpc, "Histogram", request, response,
                            get_histogram_hook, None)
  ...
  def CheckValidIntParameter(parameter, min_value, max_value, name):
    ...
    if parameter is not None:
      ...
      if parameter > max_value or parameter < min_value:
        raise BadRequestError("%s must be between %s and %s." % name,
...
def _make_async_call(rpc, method, request, response, get_result_hook,
                     user_data):
  if rpc is None:
    rpc = create_rpc()
...
def resize(image_data,
  ...
  Args:
    ...
    correct_orientation: A value from `ORIENTATION_CORRECTION_TYPE` to indicate
...
def resize_async(image_data,
  ...
  return image.execute_transforms_async(
      output_encoding=output_encoding,
      quality=quality,
      rpc=rpc,
      transparent_substitution_rgb=transparent_substitution_rgb)
...
def horizontal_flip_async(image_data,
                          output_encoding=PNG,
                          quality=None,
                          correct_orientation=UNCHANGED_ORIENTATION,
                          rpc=None,
...
def vertical_flip_async(image_data,
                        output_encoding=PNG,
                        quality=None,
                        correct_orientation=UNCHANGED_ORIENTATION,
                        rpc=None,
...
IMG_SERVING_SIZES = [
    32, 48, 64, 72, 80, 90, 94, 104, 110, 120, 128, 144, 150, 160, 200, 220,
...
def get_serving_url_async(blob_key,
  ...
  request = images_service_pb2.ImagesGetUrlBaseRequest()
  response = images_service_pb2.ImagesGetUrlBaseResponse()

  request.blob_key = _blob_key

  if secure_url:
    request.create_secure_url = secure_url
  ...
  return _make_async_call(rpc, "GetUrlBase", request, response,
                          get_serving_url_hook, None)
...
def delete_serving_url_async(blob_key, rpc=None):
  ...
  request = images_service_pb2.ImagesDeleteUrlBaseRequest()
  response = images_service_pb2.ImagesDeleteUrlBaseResponse()

  request.blob_key = _extract_blob_key(blob_key)
  ...
  return _make_async_call(rpc, "DeleteUrlBase", request, response,
                          delete_serving_url_hook, None)
