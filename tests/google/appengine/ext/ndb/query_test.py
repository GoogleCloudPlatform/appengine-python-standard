...
class BaseQueryTestMixin(object):
  ...
  def testQueryAttributes(self):
    ...
    self.assertEqual('Foo', q.kind)
    self.assertEqual(q.ancestor, key)
    self.assertEqual(query.FilterNode(b'rate', '=', 1), q.filters)
    self.assertEqual(q.orders, None)
  ...
  def testQueryRepr(self):
    ...
    q = Foo.query()
    self.assertEqual("Query(kind='Foo')", repr(q))

    q = Foo.query(ancestor=model.Key('Bar', 1))
    self.assertEqual("Query(kind='Foo', ancestor=Key('Bar', 1))", repr(q))
    ...
    self.assertEqual("Query(app='a', namespace='ns', kind='Foo')", repr(q3))
    ...
    if six.PY2:
      ...
      self.assertEqual(
          "Query(kind='Foo', projection=[b'name', b'tags'], "
          "group_by=[b'name', b'tags'])",
          repr(q5))
  ...
  def testModernQuerySyntax(self):
    ...
    expected = query.ConjunctionNode(
        query.FilterNode(b'Age', '>=', 42), query.FilterNode(b'rank', '<=', 5))
    self.assertEqual(expected, q.filters)
  ...
  def testAndQuery(self):
    ...
    self.assertEqual(q.filters, query.FilterNode(b'Age', '>=', 42))
  ...
  def testSingletonInFilter(self):
    ...
    self.assertEqual(query.FilterNode(b'name', '=', b'xyzzy'), q.filters)
    self.assertNotEqual(q.filters, 42)
  ...
  def testInFilter(self):
    ...
    expected = query.DisjunctionNode(
        query.FilterNode(b'name', '=', b'a'),
        query.FilterNode(b'name', '=', b'b'))
    self.assertEqual(expected, q.filters)
    a = Employee(name='a')
    a.put()
    b = Employee(name='b')
  ...
  def testIsDistinct(self):
    ...
    qrys = [
        Foo.query(projection=[Foo.p, 'q'], distinct=True),
        Foo.query(projection=[Foo.p, 'q'], group_by=(Foo.q, 'pp', Foo.p))]
    for qry in qrys:
      self.assertEqual(True, qry.is_distinct)

    qrys = [
        Foo.query(),
        Foo.query(projection=[Foo.p, 'q'])]
    for qry in qrys:
      self.assertEqual(False, qry.is_distinct)
  ...
  def testIndexOnlyPropertyListNormalization(self):
    ...
    def assertNormalization(expected, value):
      ...
      self.assertEqual(expected, q1.group_by)
      self.assertEqual(expected, q1.projection)
      self.assertEqual(expected, q2.group_by)
      self.assertEqual(expected, q2.projection)

    for value in ((b'pp',), [b'pp']):
      assertNormalization((b'pp',), value)

  def testIndexOnlyPropertyValidation(self):
    self.ExpectWarnings()

    class Foo(model.Model):
      p = model.IntegerProperty('pp', indexed=False)
      q = model.IntegerProperty(required=True)
  ...
  def testGroupByQuery(self):
    ...
    qry = Foo.query(group_by=[Foo.r, Foo.p],
                    default_options=query.QueryOptions(projection=[b'pp']))
    qry = qry.order(Foo.p, Foo.r, Foo.q)
    self.assertEqual(expected, [(ent.p, ent.key) for ent in qry.fetch()])
    self.assertEqual(expected, [(ent.p, ent.key) for ent in qry])
  ...
  def testProjectionQuery(self):
    ...
    self.assertCountEqual(ent._projection, ('pp', 'q'))
  ...
  def testProjectionQuery_AllTypes(self):
    ...
    boo.put()
    qry = Foo.query()
    for prop in six.itervalues(Foo._properties):
      ...
      for otherprop in six.itervalues(Foo._properties):
  ...
  def testProjectionQuery_ComputedProperties(self):
    ...
    self.assertEqual((p.c, p.d), (b'<a.b>', b'<a>'))
  ...
  def testProjectionQuery_StructuredProperties(self):
    ...
    self.assertCountEqual(x.middle.inner._projection, ('bar', 'foo'))
    self.assertCountEqual(x.middle._projection, ('inner.bar', 'inner.foo'))
    self.assertCountEqual(x._projection, ('mid.inner.bar', 'mid.inner.foo'))
    self.assertEqual(x,
                     Outer(key=one.key,
  ...
  def testFilterRepr(self):
    ...
    if six.PY2:
      self.assertEqual("FilterNode('name', '=', 'xyzzy')", repr(f))
    else:
      self.assertEqual("FilterNode(b'name', '=', b'xyzzy')", repr(f))
  ...
  def testQueryAncestorConsistentWithAppId(self):
    ...
    self.assertEqual(self.APP_ID, a.app())
    Employee.query(ancestor=a, app=a.app()).fetch()
  ...
  def testQueryAncestorConsistentWithNamespace(self):
    ...
    self.assertEqual('ns', a.namespace())
  ...
  def testQueryFilterAndOrderPreserveNamespace(self):
    ...
    self.assertEqual('ns', q2.namespace)

    q3 = q2.order(Employee.name)
    self.assertEqual('ns', q3.namespace)
  ...
  def testSkippedResultCursor(self):
    ...
    self.assertEqual('C', next(it).name)
  ...
  def testLargeCount(self):
    ...
    for i in range(0, datastore_stub_util._MAX_QUERY_OFFSET + 10):
      Bar(id=str(i)).put()
  ...
  def testFullDistributiveLaw(self):
    ...
    expected = DisjunctionNode(
        ConjunctionNode(FilterNode(b'tags', '=', b'jill'),
                        FilterNode(b'rate', '=', 1)),
        ConjunctionNode(FilterNode(b'tags', '=', b'jill'),
                        FilterNode(b'rate', '=', 2)),
        ConjunctionNode(FilterNode(b'tags', '=', b'hello'),
                        FilterNode(b'rate', '=', 1)),
        ConjunctionNode(FilterNode(b'tags', '=', b'hello'),
                        FilterNode(b'rate', '=', 2)))
  ...
  def testBlobQuery(self):
    ...
    a = MyModel(b=b'\xff\x00')
    a.put()
    q = MyModel.query(MyModel.b == b'\xff\x00')
    it = iter(q)
    b = next(it)
    self.assertEqual(a, b)
  ...
  def testExpandoQueries(self):
    ...
    for name, value in six.iteritems(testdata):
      foo = Foo()
  ...
  def testQueryCacheInteraction(self):
    ...
    b = Bar.query().get(use_cache=False)
  ...
  def testGqlAncestor(self):
    ...
    key_str = six.ensure_text(key.urlsafe())
    qry = query.gql("SELECT * FROM Foo WHERE ANCESTOR IS KEY('%s')" % key_str)
    self.assertEqual(qry.kind, 'Foo')
  ...
  def testGqlFilter(self):
    ...
    self.assertEqual(qry.ancestor, None)
    expected = query.ConjunctionNode(
        query.FilterNode(b'name', '=', b'joe'),
        query.FilterNode(b'rate', '=', 1))
    self.assertEqual(expected, qry.filters)
    self.assertEqual(qry.orders, None)
  ...
  def testGqlKeyFunction(self):
    ...
    key_str = six.ensure_text(self.joe.key.urlsafe())
    self.assertEqual(
        [joeref],
        Bar.gql("WHERE ref = KEY('%s')" % key_str).fetch())
    self.assertEqual(
        [joeref],
    ...
    self.assertEqual(
        [joeref],
        Bar.gql("WHERE ref = KEY(:1)").bind(key_str).fetch())
    self.assertEqual(
        [joeref],
  ...
  def testGqlKeyFunctionAncestor(self):
    ...
    self.assertEqual(
        [joebar],
        Bar.gql("WHERE ANCESTOR IS KEY('%s')" % key_str).fetch())
    self.assertEqual(
        [joebar],
    ...
    self.assertEqual(
        [joebar],
        Bar.gql("WHERE ANCESTOR IS KEY(:1)").bind(key_str).fetch())
    self.assertEqual(
        [joebar],
  ...
  def testGqlSubclass(self):
    ...
    self.assertIsInstance(q, MyQuery)

    qb = q.bind('joe')
    self.assertIsInstance(qb, MyQuery)

    qf = q.filter(Foo.rate == 1)
    self.assertIsInstance(qf, MyQuery)
    self.assertEqual(qf.default_options, q.default_options)

    qo = q.order(-Foo.name)
    self.assertIsInstance(qo, MyQuery)
  ...
  def testGqlUnusedBindings(self):
    ...
    qb = q.bind(self.joe.key, 2, foo=42)
  ...
  def testGqlGroupBy(self):
    ...
    self.assertEqual((b'name', b'tags'), q.projection)
    self.assertEqual((b'name', b'tags'), q.group_by)
    self.assertEqual(True, q.is_distinct)
    ents = q.fetch()
  ...
  def testGqlProjection(self):
    ...
    self.assertEqual((b'name', b'tags'), q.projection)
    self.assertEqual(None, q.group_by)
    self.assertEqual(False, q.is_distinct)
    ents = q.fetch()
    ents.sort(key=lambda ent: ent.tags)
    self.assertEqual(ents, [Foo(name='jill', tags=['jack'],
                                key=self.jill.key,
  ...
  def testQueryPickleFilter(self):
    for protocol in (0, pickle.DEFAULT_PROTOCOL):
      ...
      self.assertIsInstance(new_q, query.Query)
  ...
  def testQueryPickleParameterAndConjunction(self):
    for protocol in (0, pickle.DEFAULT_PROTOCOL):
      ...
      self.assertIsInstance(new_q, query.Query)
      self.assertEqual(new_q.filters, q.filters)
      self.assertIsInstance(new_q.filters, query.ConjunctionNode)
      self.assertIsInstance(list(new_q.filters)[0], query.ParameterNode)
  ...
  def testNodeFilterHashable(self):
    filter_node = query.FilterNode(b'rate', '=', 1)
    try:
      hash(filter_node)
    except TypeError:
      self.fail("query.FilterNode is unhashable type")
