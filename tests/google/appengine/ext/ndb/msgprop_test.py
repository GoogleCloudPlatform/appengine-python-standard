...
class MsgPropTests(test_utils.NDBTest):
  ...
  def testReprMessageProperty(self):
    name_prefix = '' if six.PY2 else 'b'
    greet1 = msgprop.MessageProperty(Greeting, 'foo')
    self.assertEqual(
        repr(greet1), "MessageProperty(Greeting, {}'foo')".format(name_prefix))
    greet2 = msgprop.MessageProperty(Greeting, 'foo', protocol='protojson')
    self.assertEqual(
        repr(greet2),
        "MessageProperty(Greeting, {}'foo', protocol='protojson')".format(
            name_prefix))
    greet3 = msgprop.MessageProperty(Greeting, 'foo', indexed_fields=['text'])
    self.assertEqual(
        repr(greet3),
        "MessageProperty(Greeting, {}'foo', indexed_fields=('text',))".format(
            name_prefix))
    greets = msgprop.MessageProperty(Greeting, 'foo', repeated=True)
    self.assertEqual(
        repr(greets),
        "MessageProperty(Greeting, {}'foo', repeated=True)".format(name_prefix))

  def testReprEnumProperty(self):
    name_prefix = '' if six.PY2 else 'b'
    color = msgprop.EnumProperty(Color, 'bar')
    self.assertEqual(
        repr(color), "EnumProperty(Color, {}'bar')".format(name_prefix))
    colors = msgprop.EnumProperty(Color, 'bar', repeated=True)
    self.assertEqual(
        repr(colors),
        "EnumProperty(Color, {}'bar', repeated=True)".format(name_prefix))
  ...
  def testErrors(self):
    ...
    self.assertRaises(ValueError, msgprop.MessageProperty,
                      Greeting, indexed_fields=['text', 'text'])
  ...
  def testBytesField(self):
    ...
    bg = BytesGreeting(data=b'\xff', when=123)
    st = Store(greet=bg)
    st.put()
    res = Store.query(Store.greet.data == b'\xff').fetch()
  ...
  def testDoubleNestedRepeatErrors(self):
    ...
    msgprop.MessageProperty(Inner, repeated=True)
    msgprop.MessageProperty(Outer, repeated=True)
  ...
  def testEnumProperty(self):
    ...
    self.assertEqual(foobar1.color, Color.BLUE)
    foobar1.put()
    ...
    self.assertRaises(TypeError, Foo, color='RED')
    self.assertRaises(TypeError, Foo, color=620)
  ...
  def testProtocolChangeProtoJsonFirst(self):
