...
class KeyPickleTests(test_utils.NDBTest):
  ...
  def testPickleBackwardsCompatibility(self):
    for pkey_list in self.pkeys:
      for expected, pkey in zip(self.keys, pkey_list):
        expected.app()
        actual = self._Unpickle(six.ensure_binary(pkey))
  ...
  def testPickleSetState(self):
    py2_pickle = b'\x80\x02ctitanoboa_apphosting.ext.ndb.key\nKey\nq\x01}q\x02(U\x05pairsq\x03U\x05Classq\x04U\x02idq\x05\x86\x85q\x06U\tnamespaceq\x07U\x00U\x03appq\x08U\x18s~kintaro-content-serverq\tu\x85\x81q\n}q\x0b(h\x03h\x06h\x07U\x00h\x08h\tu\x85b.'
    expected = key.Key('Class', 'id')
    actual = pickle.loads(py2_pickle)
